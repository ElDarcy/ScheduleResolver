...
class AiScheduleService extends ChangeNotifier {
  ...
  final String _apiKey = 'AIzaSyBFhLhR-6AE4-ymHgmjICiOYC1VQJDJvcU';
  ...
  Future<void> analyzeSchedule(List<TaskModel> tasks) async {
    ...
    try {
      ...
      final prompt =   '''
      Your are an expert student scheduling assistant. The user has provided the following task for their day in JSON format: $taskJson
      
      Your job is to analyze these task, identify any overlaps or conflicts in their start and end times,
      ...
      4. ### Explanation
      Explanation why this recommendation was made in simple language that a student would easily understand.
  ...
  ScheduleAnalysis _parseResponse(String fullText) {
    ...
    for (var section in sections) {
      ...
      if (section.startsWith('Detected Conflicts')) {
        ...
        rankedTasks = section.replaceFirst('Ranked Task', '').trim();
